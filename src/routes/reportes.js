...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const [prestamos, pagosHoy, pagosMes, totalClientes] = await Promise.all([
      Prestamo.find({ empresaId: eId }),
      Pago.find({ empresaId: eId, fechaPago: { $gte: inicioDia, $lt: finDia } }),
      Pago.find({ empresaId: eId, fechaPago: { $gte: inicioMes } }),
      Cliente.countDocuments({ empresaId: eId, activo: true }),
    ]);
    ...
    tipos.forEach(t => { prestamosPorTipo[t] = prestamos.filter(p => p.tipoPrestamo === t && p.estado !== 'pagado').length; });
...
router.get('/cuadre', async (req, res) => {
  try {
    ...
    const pagos  = await Pago.find({ empresaId: req.empresaId, fechaPago: { $gte: inicio, $lt: fin } })
